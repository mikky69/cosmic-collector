...
class TokenBattleGame {
    ...
    calculateScore() {
        const baseScore = 100;
        const levelMultiplier = this.level * 1.5;
        const timeBonus = Math.floor(this.timeLeft * 2);
        const difficultyBonus = this.level > 3 ? 50 : 0;
        
        return Math.floor((baseScore + timeBonus + difficultyBonus) * levelMultiplier);
    }
    
    checkLevelProgression() {
        // Level up every 5 correct answers
        const newLevel = Math.floor(this.correctAnswers / 5) + 1;
        
        if (newLevel > this.level) {
            this.level = newLevel;
            
            // Increase difficulty
            this.baseTime = Math.max(15, this.baseTime - 2); // Less time per question
            this.timeLeft += 10; // Bonus time for leveling up
            
            console.log(`Token Battle Level ${this.level}! Time reduced to ${this.baseTime}s per question`);
    ...
    showBattleResults() {
        ...
        stats.innerHTML = `
            ...
            <p><strong>Questions Answered:</strong> ${this.totalQuestions}</p>
            <p><strong>Correct Answers:</strong> ${this.correctAnswers}</p>
        ...
        if (window.leaderboard) {
            window.leaderboard.updateScore('tokenBattle', this.score, {
                level: this.level,
                accuracy: accuracy,
        ...
        if (window.earningSystem) {
            window.earningSystem.addEarnings('tokenBattle', this.score);
        }
    ...
    updateScore() {
        ...
        const accuracyElement = document.getElementById('battle-accuracy');
