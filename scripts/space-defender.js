...
class SpaceDefenderGame {
        ...
        this.bullets.forEach((bullet, bulletIndex) => {
            this.asteroids.forEach((asteroid, asteroidIndex) => {
                ...
                if (distance < asteroid.size) {
                    ...
                    // Increase score with progressive difficulty
                    const baseScore = 50;
                    const waveBonus = this.wave * 25;
                    const totalScore = baseScore + waveBonus;
                    this.score += totalScore;
                    
                    // Check for wave/level progression
    ...
    checkWaveProgression() {
        ...
        if (this.asteroids.length === 0) {
            ...
            this.asteroidSpeed += 10; // Faster asteroids
            this.asteroidSpawnRate += 2; // More asteroids
            
            console.log(`Wave ${this.wave} - Difficulty increased!`);
        }
        
        // Level progression every 3 waves
        const newLevel = Math.floor(this.wave / 3) + 1;
        if (newLevel > this.level) {
            this.level = newLevel;
            this.turret.fireRate = Math.max(100, this.turret.fireRate - 50); // Faster shooting
            console.log(`Level ${this.level}! Fire rate increased!`);
        }
    }
