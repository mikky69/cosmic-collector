        ...
        this.bullets.forEach((bullet, bulletIndex) => {
            ...
            this.enemies.forEach((enemy, enemyIndex) => {
                if (this.isColliding(bullet, enemy)) {
                    ...
                    // Level up every 500 points (faster progression)
                    if (this.score >= this.level * 500) {
                        this.level++;
                        
                        // Increase difficulty with each level
                        this.enemySpeed += 0.5;
                        this.enemySpawnRate *= 1.2; // More enemies
                        this.health = Math.min(this.health + 25, 100); // Health bonus
                        
                        console.log(`Level ${this.level}! Difficulty increased`);
                    }
                }
            });
        });
